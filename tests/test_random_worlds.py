...
def test_pairs_reproduce_their_output():
    ...
    for out, src in worlds:
        printed, _status, _stack, visited = bf.run(src, max_steps=2000)
        printed = [int(t) for t in _INT.findall(printed)]
        ...
        # observability filter: path short relative to observed terms
        assert int(visited.sum()) <= min(6 * len(out), 64)
